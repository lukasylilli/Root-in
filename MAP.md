...
- 🔒 **Gemeinsames Supabase-Projekt mit Vox (Stand 2026-09-16):** Vox hat eigene Tabellen (`vox_backups`, Datei `supabase/vox_tables.sql` im **Vox**-Repo) und fasst `profiles`/`backups` von Root-in nie an. Vox-Änderungen an seiner Nutzlast (zuletzt S.6, Fassung 3) und an seinen Wortdaten (V.2 Wortindex, L.1a id-Wächter, L.1b Migrationstest) brauchen hier **keine** Anpassung; `version` in der Hülle zählt je App getrennt.
