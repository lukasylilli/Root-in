...
>
> 🗓️ **Letzte Sitzung:** 2026-09-26 (später) — **Ursache der grauen Seite gefunden und behoben** ([31.8](#318-eigener-namensraum-im-browser-speicher--2026-09-26)). Die neue Fehlerseite (31.7) zeigte beim Nutzer: `TypeError: 2: type 'int' is not a subtype of type 'String?'`. **Grund:** Root-in und Vox liegen auf derselben Herkunft `lukasylilli.github.io` und teilten sich deshalb den Browser-Speicher; beide benutzen den Schlüssel `theme_mode` — Vox als Zahl (`2` = dunkel), Root-in als Text. Wer Vox auf dunkel stellt, bei dem stürzte Root-in ab (und umgekehrt Vox). **Behoben:** Root-in speichert jetzt unter eigener Vorsilbe `root_in.` (`lib/core/services/prefs_namespace/`), alte eigene Einträge werden einmalig übernommen, fremde (Vox' `theme_mode = 2`) bleiben unberührt. Test `test/unit/legacy_preference_ownership_test.dart`, Browser-Durchgang prüft `root_in.onboarding_seen`. ⏭️ Nutzer: Seite neu laden. Vox braucht dieselbe Absicherung (vox/PLAN.md).
>
> 🗓️ **Vorherige Sitzung:** 2026-09-26 — **Nutzermeldung: Seite bleibt grau** ([31.7](#317-graue-seite-fehlerseite-und-keine-browser-übersetzung--2026-09-26)). Ein Nutzer (Android, geöffnet im In-App-Browser) sah nur eine graue Fläche; alle anderen Nutzer nicht betroffen. Befund aus seinem Bildschirmfoto: Grau ist Flutters **Fehlerfläche der veröffentlichten Fassung** (ein Fehler beim Aufbau, halbdurchsichtig über dem Grün von `index.html`), und der Titel lautete „ریشه در" — **der Browser hatte die Seite maschinell übersetzt** (im Test mit Chromium nachgestellt: genau dieser Titel). Die Ursache des Fehlers selbst ist **nicht bewiesen**. Umgesetzt: (1) `web/index.html` → `translate="no"` + `<meta name="google" content="notranslate">`; (2) neue Fehlerseite `lib/core/widgets/app_error_view.dart` (dreisprachig, Fehlertext, „Neu laden") statt stummer grauer Fläche, eingebunden in `main.dart`; Helfer `lib/core/services/page_reload/`; Test `test/widget/app_error_view_test.dart`. ⏭️ **Nächster Schritt:** Lukas fragt den Nutzer, ob die Seite jetzt lädt — falls nicht, schickt der Nutzer ein Foto der neuen Fehlerseite (der Fehlertext darauf ist die Ursache). Übrige offene Punkte unverändert: brauchen Lukas.
>
> 🗓️ **Vorherige Sitzung:** 2026-09-26 (später) — an Root-in **nichts geändert**. Vermerk: in `vox` wurden 300 weitere Wortkarten angelegt (Runden 100–129, auffallend … bewohnbar, Archiv jetzt 886; nur `assets/vocab/`, Wortliste, `tool/naechste_woerter.dart`, PLAN/MAP). Keine Berührung mit Root-in oder dem geteilten Supabase-Projekt.
...
>
> 🗓️ **Vorherige Sitzung:** 2026-09-24 — an Root-in **nichts geändert**; nur vermerkt: `vox/PLAN.md` hat eine neue Schlussphase **„🌍 فاز Z“** (Entscheidung Lukas) — erst wenn alle Teile fertig und alle Tests grün sind: Z.1 echter Telegram-Bot (kein Mini App) ⇒ Z.2 Google-Werbung (AdSense) auf der Website ⇒ Z.3 Android- und iPhone-Version. **Berührt Root-in möglicherweise:** Ob Root-in ebenfalls Werbung bzw. eine Android-/iOS-Fassung bekommt (Phase 28 hat Android/iOS entfernt) und ob der Bot das geteilte Supabase-Projekt nutzt, entscheidet Lukas erst vor dem jeweiligen Schritt. Details: vox/PLAN.md → „🌍 فاز Z“.
>
> 🗓️ **Vorherige Sitzung:** 2026-09-23 (Runde 14) — **Start-Absturz bei ungültiger Browsersprache behoben** (neu: `web/locale_guard.js`, eingebunden in `web/index.html` vor `flutter_bootstrap.js`). **Warum:** Mit Chromium auf der Live-Seite nachgestellt — meldet der Browser eine Sprache, die kein gültiges BCP-47 ist (Chromium/Linux mit POSIX-Locale: `en-US@posix`), wirft die **Flutter-Engine selbst** `RangeError: Incorrect locale information provided` (`new Intl.Locale`) noch vor dem ersten Dart-Code; die Seite bleibt leer. Das Skript bereinigt die Liste nur in diesem Fall (`en-US@posix`→`en-US`, leer ⇒ `["en"]`); normale Browser bleiben unberührt. Derselbe Fehler bestand in Vox (L.3b), dieselbe Datei dort. Kein Dart-Code geändert. ⏭️ Offene Punkte unverändert: brauchen Lukas.
>
...
#### 31.7 Graue Seite: Fehlerseite und keine Browser-Übersetzung ✅ *(2026-09-26)*
- [x] **Anlass (Nutzermeldung über Lukas):** Bei **einem** Nutzer bleibt die Seite grau, alle anderen können Root-in normal nutzen. Bildschirmfoto: Android, In-App-Browser (Custom Tab), Titel „ریشه در".
- [x] **Befund:** (a) Die Farbe (≈ #B9BDBC) ist Flutters halbdurchsichtige **Fehlerfläche der veröffentlichten Fassung** über dem Grün `#2E7D5B` aus `index.html` — also lief die App an, und beim Aufbau der Seite flog ein Fehler; die veröffentlichte Fassung zeigt dann nur Grau, ohne jeden Hinweis. (b) „ریشه در" ist die **maschinelle Übersetzung** von „Root-in": Mit Chromium und Googles Übersetzungsskript auf der Live-Seite nachgestellt — genau dieser Titel. Der Browser bietet die Übersetzung an, weil `index.html` `lang="de"` meldet und das Gerät Persisch ist.
- [x] ⚠️ **Nicht bewiesen:** dass die Übersetzung den Fehler **auslöst**. In der Nachstellung (Übersetzungsskript statt eingebauter Chrome-Übersetzung) blieb die App heil. Nach Lehre 33 wurde deshalb **nichts auf Verdacht** an der App-Logik geändert; beide Maßnahmen sind unabhängig vom Befund richtig.
- [x] **Maßnahme 1 — keine Browser-Übersetzung:** `web/index.html` → `<html … translate="no">` und `<meta name="google" content="notranslate">`. Root-in hat einen eigenen Sprachschalter (DE/EN/FA); eine Maschinenübersetzung darüber verfälscht nur (Name, Abstände, DOM-Eingriffe in die Flutter-Seite).
- [x] **Maßnahme 2 — Fehlerseite statt Grau:** `lib/core/widgets/app_error_view.dart` (`installAppErrorView()`, aufgerufen am Anfang von `main()`, nur bei `kReleaseMode`). Zeigt in Persisch/Deutsch/Englisch „Etwas ist schiefgelaufen", den **Fehlertext** (für ein Foto an Lukas) und einen Knopf **„Neu laden"** (`lib/core/services/page_reload/`, bedingter Export wie `web_storage/`). Stützt sich auf nichts aus der App (kein Theme, keine Übersetzungen, kein Material) — sie erscheint ja gerade, wenn darüber etwas kaputt ist. Kleine Flächen (< 260 × 320, z. B. eine Kachel) behalten Flutters schlichte Fehlerfläche, damit nichts überläuft.
- [x] **Test:** `test/widget/app_error_view_test.dart` (2 Fälle: volle Fläche mit Text + Knopf; kleine Fläche ⇒ `ErrorWidget`, kein Überlauf).
- [x] **Ergebnis:** Die Fehlerseite hat die Ursache sofort benannt ⇒ 31.8. ~~Offen (Lukas ↔ Nutzer): Lädt die Seite jetzt? Falls nicht: Foto der neuen Fehlerseite — der Text darauf nennt die echte Ursache; erst dann an der Ursache arbeiten. Sofort-Hilfe für den Nutzer: im Browser-Menü ⋮ die Übersetzung ausschalten bzw. „Originalseite anzeigen".~~ Die Übersetzung war **nicht** die Ursache; `notranslate` bleibt trotzdem (eigener Sprachschalter).

#### 31.8 Eigener Namensraum im Browser-Speicher ✅ *(2026-09-26)*
- [x] **Befund (Fehlerseite des Nutzers):** `ProviderException … TypeError: 2: type 'int' is not a subtype of type 'String?'`. Root-in liest `theme_mode` als Text; im Speicher stand die Zahl `2`.
- [x] **Ursache:** `localStorage` gilt je **Herkunft** (`lukasylilli.github.io`), nicht je Pfad — Root-in (`/Root-in/`) und Vox (`/vox/`) teilten ihn. `shared_preferences` legt ohne eigene Vorsilbe alles unter `flutter.<name>` ab. Einziger gemeinsamer Name: `theme_mode` — Vox schreibt `ThemeMode.index` als **Zahl** (`2` = dunkel), Root-in `ThemeMode.name` als **Text**. Betroffen: wer beide Apps im selben Browser nutzt und in einer davon das Farbschema gesetzt hat. Deshalb nur ein Gerät.
- [x] **Behoben:** `lib/core/services/prefs_namespace/` — `openRootInPreferences()` (einziger Einstieg, in `main.dart` und `main_seed.dart`) setzt `SharedPreferences.setPrefix('root_in.')`. Davor übernimmt `migrateLegacyPreferences` (nur Browser, bedingter Export) jeden alten `flutter.<name>`, der nach `isRootInLegacyEntry` Root-in gehört (**Name UND Art des Werts**: `theme_mode` nur als Text, Merker nur als bool, Anleitungs-Cache nur als Text, Dashboard nur als Liste), nach `root_in.<name>` und entfernt ihn unter dem alten Namen. Vox' Einträge bleiben unberührt. Fehler beim Speicherzugriff werden geschluckt — dann Standard-Einstellungen, nie kein Start.
- [x] **Datenbank nicht betroffen:** IndexedDB heißt `root_in_db` bzw. in Vox `vox` — verschieden.
- [x] ⚠️ **Nicht geändert, aber vermerkt:** Beide Apps nutzen dasselbe Supabase-Projekt ⇒ derselbe Anmelde-Schlüssel (`sb-<ref>-auth-token`) im gemeinsamen `localStorage`. An-/Abmelden in einer App gilt damit auch in der anderen. Passt zu Phase 30 („ein Konto"), ist aber Folge der gemeinsamen Herkunft, keine eigene Entscheidung — vor Phase 30 bewusst entscheiden.
- [x] **Tests:** `test/unit/legacy_preference_ownership_test.dart` (3 Fälle, u. a. Vox' `theme_mode = 2` bleibt bei Vox); Browser-Durchgang (`tool/webtest_ci.py`, `webtest.py`) verlangt jetzt `root_in.onboarding_seen`.
- [x] ⚠️ **Regel ab jetzt:** Kein Code in Root-in ruft `SharedPreferences.getInstance()` direkt — immer `openRootInPreferences()`; `setPrefix` geht nur vor dem ersten `getInstance`.
...
- **2026-09-26** — 31.8: Die Fehlerseite aus 31.7 nannte die Ursache: gemeinsamer Browser-Speicher mit Vox (gleiche Herkunft), Schlüssel `theme_mode` dort Zahl, hier Text. Root-in hat jetzt die Vorsilbe `root_in.` mit einmaliger Übernahme der eigenen alten Einträge. **Lehre:** Zwei Apps unter einer GitHub-Pages-Domain sind für den Browser **eine** Website.
- **2026-09-26** — 31.7: Nutzermeldung „Seite bleibt grau". Grau = Fehlerfläche der veröffentlichten Fassung; Titel „ریشه در" = Browser-Übersetzung (nachgestellt). Ursache nicht bewiesen ⇒ keine Änderung auf Verdacht an der Logik (Lehre 33), stattdessen `notranslate` in `index.html` und eine dreisprachige Fehlerseite mit Fehlertext und „Neu laden" — damit der **nächste** Fehler sich selbst benennt.
